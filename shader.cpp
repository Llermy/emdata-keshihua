...
void Shader::setVec3(const std::string &name, float vx, float vy, float vz) const
{
    unsigned int vecLoc = glGetUniformLocation(ID, name.c_str());
    glUniform3fv(vecLoc, 1, glm::value_ptr(glm::vec3(vx, vy, vz)));
}

void Shader::setVec3(const std::string &name, glm::vec3 vec3) const
{
    unsigned int vecLoc = glGetUniformLocation(ID, name.c_str());
    glUniform3fv(vecLoc, 1, glm::value_ptr(vec3));
}